...
class CustomScriptView(APIView):
    def get(self, request):
        ...
        temp_pass = "admin"
        for ab in data:
            ab.set_password(temp_pass)
            ab.save()
            print(ab.password)
            user = authenticate(email=ab.email,password=temp_pass)
            print(f"==>> user: {user}")
            print(user)
...
class AdminDashboardView(APIView):
    def get(self, request):
        try:
            ...
            late_logins = EmployeeAttendance.objects.filter(day=today,check_in__lte=late_time)
            ...
            data = {
                ...
                "current_birthdays": serializers.UserMiniSerializer(current_birthdays, many=True).data,
                "upcoming_leaves": serializers.LeaveMiniSerializer(upcoming_leaves, many=True).data,
                "upcoming_holidays": serializers.HolidayMiniSerializer(upcoming_holidays, many=True).data,
                "recent_joiners": serializers.UserMiniSerializer(recent_joiners, many=True).data,
                "present_employee": " ",
                "absent_employee": " ",
                "late_logins": serializers.LateLoginMiniSerializer(late_logins, many=True).data,
            }

            return ApiResponse.success(message="dashboard fetched successfully",data=data)
